...
const launchContest = () => {
    ...
    if (
        ...
        contest.value.date == null
    ) {
        console.log(contest);
        errorMessage.value =
            "Il faut remplir tous les champs du formulaire pour créer un concours !";
    } else {
...
<template>
    ...
    <BaseInput
        placeholder="Saississez le titre du concours"
        @emit-input="updateContestData($event, 'title')"
        class="concours-basInput"
    />
    <p class="titre2">Description</p>
    <BaseTextArea
        placeholder="Saisissez la description du concours"
        @emit-text-area="updateContestData($event, 'description')"
        class="concours-basInput"
    />
    <p class="titre2 texteTitre3">Type de réponses attendue</p>
